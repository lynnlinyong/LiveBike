...
- (void)viewDidLoad
{
    ...
    //add UItableView
    self.tableView=[[UITableView alloc]initWithFrame:CGRectMake(0, 64, self.view.frame.size.width, self.messageToolView.frame.origin.y-64) style:UITableViewStylePlain];
...
- (void)didSelecteShareMenuItem:(ZBMessageShareMenuItem *)shareMenuItem atIndex:(NSInteger)index
{
    switch (index) {
        ...
        case 3:
        {
            StartActiveViewController *sVctr = [[StartActiveViewController alloc]init];
...
- (void)messageViewAnimationWithMessageRect:(CGRect)rect  withMessageInputViewRect:(CGRect)inputViewRect andDuration:(double)duration andState:(ZBMessageViewState)state{
    ...
    self.tableView.frame = CGRectMake(0.0f,0.0f,320.0f,self.messageToolView.frame.origin.y);
    [self scrollTableView];
}
...
-(void)scrollTableView
{
    ...
    {
        [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:self.cellFrames.count-1 inSection:0]
